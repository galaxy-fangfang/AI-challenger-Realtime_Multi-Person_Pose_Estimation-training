...
mkdir('dataset/AI_challenge/mask2017')
...
for mode = 0:1
    ...
    if mode == 1 
        %continue;
        load('dataset/AI_challenge/mat/coco_kpt.mat');
    ...
    for i = 1:L
        if mode == 1
            img_paths = sprintf('AI_challenge/train/keypoint_train_images_20170902/%s.jpg', coco_kpt(i).img_path);
            ...
            img_name1 = sprintf('dataset/AI_challenge/mask2017/mask_all_%s.png', coco_kpt(i).img_path);
            img_name2 = sprintf('dataset/AI_challenge/mask2017/mask_miss_%s.png', coco_kpt(i).img_path);
        else
        ...
        try
            ...
            [h,w,~] = size(imread(['dataset/', img_paths]));
            mask_all = false(h,w);
            mask_miss = false(h,w);
            flag = 0;
            ...
            if mode == 1
                img_name = sprintf('dataset/AI_challenge/mask2017/mask_all_%s.png', coco_kpt(i).img_path);
                %img_name = sprintf('dataset/COCO/mask2014/train2014_mask_all_%012d.png', coco_kpt(i).image_id);
                imwrite(mask_all,img_name);
                img_name = sprintf('dataset/AI_challenge/mask2017/mask_miss_%s.png', coco_kpt(i).img_path);
