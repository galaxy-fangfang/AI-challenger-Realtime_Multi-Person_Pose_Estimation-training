...
sys.path.insert(0, os.path.join('/home/fangfang/caffe/', 'python/'))
import caffe
import os.path
import struct
	...
	for d in range(len(datasets)):
		if(datasets[d] == "MPI"):
			...
			#with open('dataset/COCO/json/COCO.json') as data_file:
                        with open('dataset/AI_challenge/json/ai.json') as data_file:
                                data_this = json.load(data_file)
				data_this = data_this['root']
				data = data + data_this
			numSample = len(data)
	...
	for count in range(numSample):
		...
		if "MPI" in data[idx]['dataset']:
			...
			#path_header = '/media/posenas4b/User/zhe/Convolutional-Pose-Machines/training/dataset/COCO/images/'
                        path_header =  'dataset/AI_challenge/'
                elif "AI" in data[idx]['dataset']:
                        path_header = '/dataset/AI_challenge/'
		...
		#img_idx = data[idx]['img_paths'][-16:-3];
                img_idx = data[idx]['img_paths'][-44:-4]
		#print img_idx
		if "COCO_val" in data[idx]['dataset']:
			...
			print path_header+'mask2017/mask_miss_'+img_idx+'.png'
                        mask_all = cv2.imread(path_header+'mask2017/mask_all_'+img_idx+'.png', 0)
			mask_miss = cv2.imread(path_header+'mask2017/mask_miss_'+img_idx+'.png', 0)
                        #print path_header+'mask2017/mask_miss_'+img_idx+'.png'
...
if __name__ == "__main__":
	...
	writeLMDB(['COCO'], '/home/fangfang/PycharmProjects/Realtime_Multi-Person_Pose_Estimation/training/AI_kpt/lmdb', 1)
