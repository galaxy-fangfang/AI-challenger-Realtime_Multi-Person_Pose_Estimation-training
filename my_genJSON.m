function genJSON(dataset)
    ...
    if(strcmp(dataset, 'AI'))
        mkdir('dataset/AI_challenge/json')
        count = 1;
        makeFigure = 0;
        validationCount = 0;
        isValidation = 0;
        ...
        %load('dataset/AI_challenge/mat/coco_val.mat');
	    ...
	    %mode = 1;
            ...
            trainIdx = 1:1:size(RELEASE,2);%45174
            ...
            for i = trainIdx
                ...
                for p = 1:numPeople
                    ...
                    person_center = [RELEASE(i).annorect(p).neck{1}(1),RELEASE(i).annorect(p).neck{1}(2)]
                    flag = 0;
                    ...
                    if mode == 0
                        ...
                        %joint_all(count).img_paths = sprintf('train/keypoint_train_images_20170902/%s.jpg', RELEASE(i).annorect(p).img_path);
                        joint_all(count).img_paths = sprintf('train/keypoint_train_images_20170902/%s.jpg', RELEASE(i).img_path);
                    else
        ...
        opt.FileName = 'dataset/AI_challenge/json/ai.json';
        opt.FloatFormat = '%.3f';
